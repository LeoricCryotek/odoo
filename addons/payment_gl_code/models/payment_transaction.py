...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        self._handle_feedback_data('glcode', {'reference': self.reference})
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        tx = self.search([('reference', '=', reference), ('provider', '=', 'glcode')])
        if not tx:
            raise ValidationError(
                "GL Code: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
